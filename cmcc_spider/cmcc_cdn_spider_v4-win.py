"""
中国移动CDN - 带宽统计XLS导出脚本 (v4-win)
功能：登录CDN平台，自动导出昨日带宽统计XLS文件（最小单位，1000进制）

改动说明（基于v4，适配Windows环境）：
1. 配置外置到YAML文件，支持多客户（一个文件配置多个客户）
2. 移除Linux容器特有参数（--no-sandbox、--disable-dev-shm-usage）
3. 默认下载目录改为Windows用户Downloads文件夹
4. 支持通过配置文件指定chromedriver路径（Windows常见需求）

用法：
    python cmcc_cdn_spider_v4-win.py                    # 默认跑第一个客户
    python cmcc_cdn_spider_v4-win.py --list             # 列出所有客户
    python cmcc_cdn_spider_v4-win.py --customer "淮北移动"  # 指定客户
    python cmcc_cdn_spider_v4-win.py --all              # 批量执行所有客户
...
def load_config(config_path, customer_name=None):
    ...
    配置格式：
        ...
        defaults:
          unit_scale: 1000
          download_dir: "."
          chromedriver_path: null   # Windows下可指定chromedriver.exe路径
    """
...
def main():
    parser = argparse.ArgumentParser(description='CDN带宽统计XLS导出脚本 v4-win (Windows版)')
