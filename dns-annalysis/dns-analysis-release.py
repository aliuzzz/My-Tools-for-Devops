...
# 归属地数据文件路径
HOME_DAT_PATH = '/root/dns-analysis/qqwry.dat'
# 输入文件路径
SOURCE_PATH = '/data/mtr/'+year+'年'+month1+'月/'+year+'年'+month1+'月'+day+'日/域名测试/华为域名测试'
# 输出文件路径
...
def gen_result(res, res_2):
    ...
    with open(RESULT_2_PATH, 'a+') as f:
        f.write(res_2) 
    logging.basicConfig(
        filename='/var/log/dns-analysis.log',
        level=logging.DEBUG,
        format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]'
    )
    logging.debug('hw-result1完成')
...
def add_home(di):
    ...
    for ip, count in di.items():
        home = find_home(ip)
        text = text + '("' + ip + '", ' + home + ', 解析次数：' + str(count) + ')' + '\n' 
    logging.basicConfig(
        filename='/var/log/dns-analysis.log',
        level=logging.DEBUG,
        format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]'
    )
    logging.debug('hw-result2完成')
    return(text)
