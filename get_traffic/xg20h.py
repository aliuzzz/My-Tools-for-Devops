...
filename = '/root/xg_download/xg20h.txt'
...
if res.get('code') == '0001':
    ...
    with open('/root/xg_download/xg20h.txt', 'w') as f:
    ...
    with open('/root/xg_download/get_traffic.sh','w') as f:
        ...
        for _ in range(int(mission_num)):
            f.write("/usr/bin/screen -dmS mysession /usr/bin/go run /root/xg_download/xg20h.go\n")
            f.write("sleep 60\n")
    os.chmod('/root/xg_download/get_traffic2.sh', 0o755)
