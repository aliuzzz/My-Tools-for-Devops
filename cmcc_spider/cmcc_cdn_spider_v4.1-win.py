"""
中国移动CDN - 带宽统计XLS导出 + 采样推送脚本 (v4.1-win)
功能：
    1. 登录CDN平台，自动导出昨日带宽统计XLS文件
    2. 解析XLS数据，转换为Mbps采样点
    3. 推送到对方系统的 /system/cdn/traffic/samples 接口

改动说明（基于v4.1，适配Windows环境）：
    1. 支持多客户配置（一个YAML文件配置多个客户）
    2. 下载XLS后自动解析为采样数据
    3. bps → Mbps 转换后推送到指定接口
    4. 移除Linux容器特有参数，默认下载目录改为Windows Downloads
    5. 支持通过配置指定chromedriver路径

用法：
    python cmcc_cdn_spider_v4.1-win.py                    # 默认跑第一个客户
    python cmcc_cdn_spider_v4.1-win.py --list             # 列出所有客户
    python cmcc_cdn_spider_v4.1-win.py --customer "淮北移动"  # 指定客户
    python cmcc_cdn_spider_v4.1-win.py --all              # 批量执行所有客户
    python cmcc_cdn_spider_v4.1-win.py --skip-push        # 只下载不推送

依赖：
    pip install selenium requests ddddocr pyyaml pandas xlrd
"""
import argparse
import os
import sys
...
def load_config(config_path, customer_name=None):
    ...
    for cust in customers:
        ...
        # v4.1 推送配置校验
        if merged.get('auto_push', True):
            push_api = merged.get('push_api', {})
            if not push_api.get('url'):
                raise ValueError(f"客户 '{merged.get('name')}' auto_push=true 时，push_api.url 为必填项")
            if not push_api.get('customer_id'):
                raise ValueError(f"客户 '{merged.get('name')}' auto_push=true 时，push_api.customer_id 为必填项")
            if not push_api.get('customer_name'):
                raise ValueError(f"客户 '{merged.get('name')}' auto_push=true 时，push_api.customer_name 为必填项")
...
def _create_driver(cfg, debug=False):
    """创建Chrome WebDriver（Windows适配）

    Args:
        debug: True 时弹出浏览器窗口（不启用 headless）
    """
    ...
    # Windows下不需要 --no-sandbox 和 --disable-dev-shm-usage（这是Linux容器环境用的）
    ...
    # 支持通过配置指定chromedriver路径（Windows常见需求）
...
def run_spider(cfg, debug=False):
    """主流程：登录 → 选择昨日 → 导出XLS

    Args:
        debug: True 时弹出浏览器窗口，方便肉眼观察操作
    """
    download_dir = cfg['download_dir']

    driver = _create_driver(cfg, debug=debug)
    xls_path = None
...
def parse_xls_to_samples(xls_path, stat_date):
    """解析XLS文件，转换为采样数据（Mbps）
    ...
    # 期望两列：时间列 + 带宽(bps)列
    if len(df.columns) < 2:
        raise ValueError(f"XLS列数不足，期望至少2列，实际{len(df.columns)}列")
    ...
    for _, row in df.iterrows():
        ...
        if pd.isna(time_str) or pd.isna(bw_bps):
            continue

        # 时间格式：判断XLS里是否已经有完整日期
        if len(time_str) >= 10 and time_str[4] == '-' and time_str[7] == '-':
            sample_time = time_str  # XLS里已经是完整日期时间，直接用
        else:
            sample_time = f"{stat_date} {time_str}"  # 只有时间部分，拼接日期
        # bps → Mbps
        bw_mbps = float(bw_bps) / 1_000_000

        samples.append({
            "sampleTime": sample_time,
            "trafficValue": round(bw_mbps, 6),
            "trafficUnit": "Mbps"
        })

    print(f"[XLS解析] 共解析 {len(samples)} 个采样点")
    return samples


def push_samples(cfg, samples, stat_date):
    """推送采样数据到对方接口"""
    push_api = cfg['push_api']
    url = push_api['url']
    customer_id = push_api['customer_id']
    customer_name = push_api['customer_name']
    extra_headers = push_api.get('headers', {})

    # 生成批次号
    batch_no = f"cdn-{stat_date.replace('-', '')}-{datetime.now().strftime('%H%M%S')}"
    ...
    payload = {
        ...
        "sourceType": "api",
        "batchNo": batch_no,
        "remark": f"CDN带宽采样数据，域名: {','.join(cfg['target_domains'])}",
        "samples": samples
    }
    ...
    print(f"[推送] 准备推送 {len(samples)} 个采样点到 {url}")
    print(f"[推送] customerId={customer_id}, statDate={stat_date}, batchNo={batch_no}")

    try:
        resp = requests.post(url, json=payload, headers=headers, timeout=60)
        result = resp.json()

        print(f"[推送] 接口返回 HTTP {resp.status_code}, body={result}")

        # 兼容多种成功判断：code=0 或 HTTP 200 且 success=true
        is_success = result.get('success') == True
        code = result.get('code')

        if is_success and (code == 0 or code == 200 or code is None):
            r = result.get('result', {})
            print(f"[推送] 成功: total={r.get('totalCount')}, created={r.get('createdCount')}, updated={r.get('updatedCount')}")
            return True
        else:
            print(f"[推送] 失败: success={result.get('success')}, code={code}, message={result.get('message')}")
            return False
    except Exception as e:
        print(f"[推送] 请求异常: {e}")
        return False
...
def main():
    parser = argparse.ArgumentParser(description='CDN带宽采样推送脚本 v4.1-win (Windows版)')
    ...
    parser.add_argument('--skip-push', action='store_true', help='跳过推送，只下载XLS')
    parser.add_argument('--debug', action='store_true', help='调试模式：弹出浏览器窗口，方便肉眼观察操作（默认 headless）')
    ...
    if args.list:
        ...
        for i, cust in enumerate(customers, 1):
            ...
            if cust.get('push_api'):
                print(f"     推送客户: {cust['push_api'].get('customer_name')}")
            print()
        return
    ...
    for cust in to_run:
        ...
        if cust.get('push_api') and not args.skip_push:
            print(f"[推送] 接口: {cust['push_api']['url']}")
            print(f"[推送] 客户: {cust['push_api']['customer_name']}")
        print('='*50)

        xls_path = run_spider(cust, debug=args.debug)
        ...
        # 推送
        if args.skip_push or not cust.get('auto_push', True):
            print(f"[{name}] 推送已跳过")
            continue

        stat_date = get_yesterday()
        try:
            samples = parse_xls_to_samples(xls_path, stat_date)
            if not samples:
                print(f"[{name}] 警告：未解析到任何采样点")
                continue

            success = push_samples(cust, samples, stat_date)
            if success:
                print(f"\n[{name}] 推送成功")
            else:
                print(f"\n[{name}] 推送失败")
